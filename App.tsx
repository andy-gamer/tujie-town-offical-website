...
// --- Assets & Constants ---
const ASSETS = {
  heroNormal: './assets/hero-normal.jpg', 
  heroHorror: './assets/hero-horror.jpg',
  townTeahouse: './assets/town-teahouse.jpg',
  storyHall: './assets/story-hall.jpg',
  monsterSketch: './assets/monster-sketch.png',
  monsterClassroom: './assets/monster-classroom.jpg',
  characterDesign: './assets/character-design.jpg',
};

const FALLBACKS = {
  heroNormal: 'https://images.unsplash.com/photo-1598556776374-92df246e96a7?q=80&w=1920&auto=format&fit=crop', // Foggy atmosphere
  heroHorror: 'https://images.unsplash.com/photo-1518562180175-34a163b1a9a6?q=80&w=1920&auto=format&fit=crop', // Red blood moon vibe
  townTeahouse: 'https://images.unsplash.com/photo-1545231499-32a2d1c09030?q=80&w=1000&auto=format&fit=crop', // Old vintage room
  storyHall: 'https://images.unsplash.com/photo-1597250861267-349e4d51cc34?q=80&w=1000&auto=format&fit=crop', // Abandoned building
  monsterSketch: 'https://placehold.co/600x800/e8dfcc/3e2b20?text=Sketch:+Tree+Man',
  monsterClassroom: 'https://images.unsplash.com/photo-1629760887885-7730829e854b?q=80&w=800&auto=format&fit=crop', // Creepy school
  characterDesign: 'https://placehold.co/400x800/transparent/transparent?text=Character', // Placeholder for character portrait
};
...
// 0. Cinematic Preloader (Silent Hill Style)
const Preloader: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  const [step, setStep] = useState(0);

  useEffect(() => {
    const sequence = [
      { time: 1000, action: () => setStep(1) }, // Show Logo
      { time: 2500, action: () => setStep(2) }, // Show Warning
      { time: 4500, action: () => setStep(3) }, // Glitch out
      { time: 5000, action: onComplete },       // Finish
    ];
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center cursor-none">
      ...
      {/* Noise Overlay */}
      <div className="absolute inset-0 pointer-events-none opacity-10 bg-[url('https://cdnjs.cloudflare.com/ajax/libs/paper-css/0.3.0/examples/assets/texture/paper.png')] mix-blend-overlay"></div>
    </div>
  );
};

// 1. Header with Anchors
...
const Hero: React.FC<HeroProps> = ({ foundItems, onFindItem }) => {
  ...
  return (
    <>
      <section 
        id="hero"
        className="relative w-full h-screen overflow-hidden bg-deep-black select-none"
      >
        {/* Concept Art Background with Ken Burns Effect */}
        <div className="absolute inset-0 w-full h-full overflow-hidden">
           <img 
            src={ASSETS.heroNormal} 
            onError={e => e.currentTarget.src = FALLBACKS.heroNormal} 
            className="w-full h-full object-cover opacity-80 animate-ken-burns" 
            alt="Concept Art" 
           />
           {/* Atmospheric Overlays */}
           <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-black/60"></div>
           <div className="absolute inset-0 bg-[url('https://raw.githubusercontent.com/danielstuart14/CSS_FOG_ANIMATION/master/fog1.png')] bg-repeat-x animate-fog opacity-40"></div>
           <div className="absolute inset-0 bg-[url('https://raw.githubusercontent.com/danielstuart14/CSS_FOG_ANIMATION/master/fog2.png')] bg-repeat-x animate-fog-slow opacity-20"></div>
        ...
        <div className="absolute bottom-24 left-6 md:left-20 max-w-4xl pointer-events-auto z-10">
           <div className="p-6">
              ...
              <button 
                ...
                className="group flex items-center gap-4 text-sm tracking-widest text-white hover:text-alert-red transition-colors"
              >
        ...
        {/* Collectible Item (Hidden in the scene) */}
        {!isFound && heroItem && (
          <button
...
// 3. Town Section (File Folder Style - Wide Layout)
  ...
  return (
    <section id="town" className="relative py-32 bg-[#1a1a1a] overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10 flex flex-col items-center">
        ...
        {/* File Folder Container (Wide Layout) */}
        <div className="relative w-full max-w-7xl flex flex-col md:flex-row gap-0 shadow-[0_25px_50px_rgba(0,0,0,0.5)]">
          ...
          {/* Left Column: The "Folder Cover" / Evidence Board */}
          <div className="w-full md:w-5/12 bg-[#d6cbb1] border border-[#c4b69c] p-10 relative flex flex-col items-center justify-center bg-[url('https://www.transparenttextures.com/patterns/paper.png')] rounded-tl-none rounded-bl-lg md:rounded-bl-lg md:rounded-tr-none">
             {/* Paper Texture Overlay */}
             <div className="absolute inset-0 opacity-30 mix-blend-multiply bg-[url('https://www.transparenttextures.com/patterns/cardboard.png')]"></div>
             ...
             <div className="relative z-10 w-full max-w-md transform rotate-1 transition-transform hover:rotate-0 duration-500">
                ...
                <div className="bg-white p-3 pb-12 shadow-md mb-8">
                  <img 
                      src={ASSETS.townTeahouse} 
                      onError={e => e.currentTarget.src = FALLBACKS.townTeahouse} 
                      className="w-full h-64 object-cover grayscale contrast-125 border border-gray-200" 
                      alt="Teahouse" 
                  />
                  <div className="absolute bottom-3 right-4 text-gray-400 font-mono text-xs">FIG. 1: THE TEA HOUSE</div>
                </div>

                {/* Character Sketch pinned */}
                <div className="absolute -bottom-10 -right-6 w-48 bg-transparent transform -rotate-6 filter drop-shadow-xl">
                    <img 
                      src={ASSETS.characterDesign} 
                      onError={e => e.currentTarget.src = FALLBACKS.characterDesign} 
          ...
          {/* Right Column: The "Document Page" */}
          <div className="w-full md:w-7/12 bg-[#f4f1ea] p-8 md:p-16 relative bg-[url('https://www.transparenttextures.com/patterns/paper.png')] border-t md:border-t-0 border-r border-b border-[#c4b69c] rounded-br-lg rounded-tr-lg shadow-inner">
             ...
             <div className="h-full p-4 md:p-8 relative border border-gray-300 bg-white shadow-sm">
                ...
                <div className="relative z-10">
                  <div className="flex justify-between items-end border-b-2 border-black pb-2 mb-6">
                    <h3 className="text-3xl md:text-4xl font-display font-bold text-black tracking-[0.2em]">
                      鎮民生活公約
                    </h3>
                  ...
                  <ol className="space-y-6 font-serif text-black text-lg list-decimal list-inside pl-2">
                    {[
                      ...
                      <li 
                        ...
                        className={`relative p-2 transition-all ${idx === 3 ? 'cursor-help border border-transparent' : ''}`}
                      >
                        {/* Rule #4 Special Styling */}
                        {idx === 3 ? (
                            <span className="relative inline-block w-full group">
                              <span className={`relative z-10 transition-opacity duration-100 ${activeRule === 3 ? 'opacity-0' : 'opacity-100'}`}>
                                {rule}
                              </span>
                              
                              {/* Trembling underline hint */}
                ...
                {!isFound && townItem && (
                  <button
                    onClick={() => onFindItem(townItem.id)}
                    className="absolute bottom-4 right-4 transform rotate-6 hover:rotate-0 hover:scale-110 transition-all z-30 group"
                    title="撿起車票"
                  >
...
const Gameplay: React.FC<{ foundItems: string[], onFindItem: (id: string) => void }> = ({ foundItems, onFindItem }) => {
  ...
  const cards = [
    { 
      title: '懷舊探索', 
      sub: 'EXPLORATION', 
      desc: '穿梭於 1970 年代的台灣街景、廢棄糖廠與老式車站。', 
      icon: 'fa-map', 
      img: ASSETS.townTeahouse 
    },
    { 
      title: '看取能力', 
      sub: 'THE SIGHT', 
      desc: '觸摸舊物讀取殘留的記憶碎片，還原被掩蓋的真相。', 
      icon: 'fa-eye', 
      img: ASSETS.storyHall 
    },
    { 
      title: '異界生存', 
      sub: 'SURVIVAL', 
      desc: '當月亮變色，面對擬態成人類的「花倀」，利用弱點求生。', 
      icon: 'fa-person-running', 
      img: ASSETS.monsterClassroom 
    },
  ];
  ...
  return (
    <section id="gameplay" className="py-32 bg-[#111] text-white relative">
       <div className="container mx-auto px-6">
          <h2 className="text-4xl font-display font-bold text-center mb-16 text-paper-yellow tracking-[0.3em]">
             核心玩法
          </h2>
             ...
             {cards.map((card, idx) => (
                <div key={idx} className="group relative h-[450px] bg-gray-900 border border-gray-800 overflow-hidden hover:border-alert-red transition-colors duration-500 flex flex-col">
                   ...
                   <div className="absolute inset-0 opacity-40 group-hover:opacity-20 transition-opacity duration-500">
                      <img src={card.img} onError={e => e.currentTarget.src = FALLBACKS.heroNormal} className="w-full h-full object-cover grayscale" alt={card.title} />
                   </div>
                   ...
                   <div className="absolute inset-0 flex flex-col justify-end p-8 bg-gradient-to-t from-black via-black/80 to-transparent">
                      ...
                      <p className="text-gray-400 text-sm font-serif leading-relaxed border-t border-gray-800 pt-4 group-hover:border-alert-red/50 transition-colors">
                        {card.desc}
                      </p>
                   </div>
...
// 5. Monster Index (Full Width Specimen Box)
    ...
    <section id="collection" className="py-24 bg-[#0a0a0a] relative overflow-hidden border-t border-gray-900 flex justify-center">
      <div className="w-full max-w-[95%] relative z-10">
        ...
        <div className="text-center mb-12">
           <h2 className="text-4xl font-display font-bold tracking-[0.3em] text-paper-yellow mb-4">
             怪物圖鑑
           </h2>
        ...
        {/* Massive Specimen Box Container */}
        <div className="relative bg-[#2a1d15] p-2 md:p-6 rounded-sm shadow-[0_30px_60px_rgba(0,0,0,0.9)] border-8 border-[#3e2b20] ring-1 ring-[#1a110d] mx-auto w-full">
           ...
           <div className="bg-[#1a110d] p-1 shadow-inner rounded-sm h-auto md:h-[700px]">
             <div className="flex flex-col md:grid md:grid-cols-4 gap-2 bg-[#2a1d15] h-full">
                ...
                {/* Left Column: Collection Slots (Responsive Fix) */}
                <div className="col-span-1 bg-[#1a110d]/50 p-4 md:p-6 flex flex-row md:flex-col gap-4 overflow-x-auto md:overflow-y-auto custom-scrollbar border-b md:border-b-0 md:border-r border-[#3e2b20] min-h-[120px] md:min-h-0">
                   <div className="text-[#8c7b6c] text-xs md:text-sm font-mono tracking-widest text-center mb-0 md:mb-2 border-r md:border-r-0 md:border-b border-[#3e2b20] pr-4 md:pr-0 pb-0 md:pb-4 flex items-center justify-center shrink-0">
                     PARTS
                   </div>
                   {ITEMS.map((item, i) => {
                ...
                {/* Right Column: Main Specimen (Massive Area) */}
                <div className="col-span-1 md:col-span-3 bg-[#e3d5b8] relative bg-[url('https://www.transparenttextures.com/patterns/cork-board.png')] shadow-inner flex flex-col md:flex-row p-6 md:p-12 overflow-hidden min-h-[500px]">
                   ...
                   {!allFound && (
                      <div className="absolute inset-0 bg-black/70 z-20 flex flex-col items-center justify-center text-center backdrop-blur-md">
                         ...
                         <div className="bg-[#1a110d] border border-alert-red/50 px-4 md:px-8 py-2 md:py-4 text-alert-red font-mono text-sm md:text-lg tracking-widest shadow-2xl">
                            ACCESS DENIED
                         </div>
                   ...
                   <div className={`transition-all duration-1000 w-full h-full flex flex-col md:flex-row gap-6 md:gap-10 ${allFound ? 'opacity-100 blur-0' : 'opacity-20 blur-sm'}`}>
                      ...
                      <div className="relative flex-1 h-64 md:h-full bg-white p-2 md:p-4 shadow-xl transform rotate-1 border border-gray-200">
                         ...
                         <img src={ASSETS.monsterSketch} onError={e => e.currentTarget.src = FALLBACKS.monsterSketch} className="w-full h-full object-contain mix-blend-multiply" alt="Monster" />
                      ...
                      <div className="relative w-full md:w-1/3 flex flex-col gap-6">
                         ...
                         <div className="bg-[#f9f5eb] p-6 md:p-8 shadow-lg transform -rotate-1 border border-[#e0d0b0] relative mt-8 md:mt-0">
                            ...
                            <p className="text-sm italic text-[#5a4a42] font-serif leading-relaxed">
                               「它……好像是我失蹤的鄰居阿明。」
                            </p>
                         </div>
...
const FloatingControls: React.FC = () => (
  <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-4">
    ...
    <a 
      ...
      target="_blank" 
      rel="noreferrer"
      className="w-12 h-12 md:w-14 md:h-14 bg-alert-red text-white rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(194,28,28,0.5)] hover:scale-110 transition-transform border-2 border-red-900 group relative"
    ...
    <button 
      ...
      className="w-12 h-12 md:w-14 md:h-14 bg-black/80 backdrop-blur-sm text-white rounded-full flex items-center justify-center shadow-lg border border-gray-700 hover:bg-gray-800 transition-colors group relative"
      title="Back to Top"
    >
...
const Footer: React.FC = () => (
  <footer className="bg-black py-16 text-center border-t border-red-900/30 relative">
    ...
    <p className="text-gray-400 font-serif text-sm tracking-widest mb-8 max-w-md mx-auto italic">
      「以源為始，稚心投入。白日凡人，夜裡追夢人。」
    </p>
...
export default function App() {
  ...
  // Auto-scroll to collection when full
  useEffect(() => {
